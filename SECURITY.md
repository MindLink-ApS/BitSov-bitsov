...
Encrypt your report using our PGP key (see [`SECURITY_KEY.asc`](./SECURITY_KEY.asc)), or use
GitHub's private vulnerability reporting for this repository (**Security → Report a
vulnerability**) — that channel needs no PGP and reaches the same maintainers.
...
> `8407 155E C7A1 F376 A910  C509 CFFC 76C3 AE38 03C4` (`security@bitsov.io`) must not be used.
> It was generated by an automated build session and its private key was never in the
> maintainers' custody, so reports encrypted to it cannot be read. No revocation certificate
> exists; if the private key is ever recovered, a revocation will be published here.
> **Do not encrypt to it.** If you sent a report to it, please re-encrypt to the key above and
> resend — we honour the original submission date.
