...
use crate::traits::lightning::{LightningProvider, PaymentStatus};
use crate::traits::pricing::{PricingEngine, PricingError};
use crate::types::{NodeId, Nonce};
...
impl PaymentGate {
    ...
    pub async fn verify(
        ...
        // applies a trust discount for reliable peers (v2.1).
        self.verify_price(envelope, pricing, trust_discount).await?;
        ...
        // (step 1) is already cryptographically sufficient.
        if self.config.verify_lightning_settlement {
            if let Some(ln) = lightning {
                self.verify_settlement(envelope, ln).await?;
    ...
    fn verify_signature(&self, envelope: &UkmEnvelope) -> Result<(), GateRejection> {
        ...
        verifying_key
            ...
            .map_err(|e: ed25519_dalek::SignatureError| GateRejection::InvalidSignature(e.to_string()))
    }
    ...
    async fn verify_price(
        ...
        let required_msat = if clamped_discount > 0.0 {
...
mod tests {
    ...
    use crate::kind::{KindCategory, KIND_CHAT};
    use crate::traits::lightning::{
        Invoice, LightningError, PaymentDetails, PaymentDirection,
    };
    ...
    async fn accept_valid_message() {
        ...
        let result = gate
            .verify(&envelope, &nonce_store, &pricing, None, None, 0.0)
            .await;
    ...
    async fn accept_zero_price_zero_payment() {
        ...
        assert!(result.is_ok(), "zero price with zero payment should be accepted");
    }
    ...
    async fn accept_slightly_future_timestamp() {
        ...
        let mut envelope =
            UkmEnvelopeBuilder::new(KIND_CHAT, sender, recipient, b"slight future".to_vec(), proof)
                .timestamp(future_ts)
                .build();
        ...
        assert!(result.is_ok(), "slightly-future timestamp should be accepted: {result:?}");
    }
    ...
    async fn reject_when_pricing_returns_chain_unavailable() {
        ...
        match &result {
            ...
            other => panic!(
                "expected PricingFailed with chain unavailable cause, got: {other:?}"
            ),
        }
    }
    ...
    async fn accept_max_u64_payment() {
        ...
        assert!(result.is_ok(), "u64::MAX payment should be accepted: {result:?}");
    }
    ...
    async fn no_whitelist_accepts_any_sender() {
        ...
        assert!(result.is_ok(), "None whitelist should accept any sender: {result:?}");
    }
    ...
    async fn timestamp_check_before_signature_check() {
        ...
        let envelope = UkmEnvelopeBuilder::new(KIND_CHAT, sender, recipient, b"old".to_vec(), proof)
    ...
    async fn accept_recent_message() {
        ...
        assert!(result.is_ok(), "recent message should be accepted, got: {result:?}");
    }
}
